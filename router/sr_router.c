...
void sr_handle_ip_pkt(struct sr_instance* sr, uint8_t* packet, unsigned int len, char* interface) 
{
    ...
    if (!sr_ip_sum_valid(packet + sizeof(struct sr_ethernet_hdr), sizeof(struct sr_ip_hdr))) {
        return;
    }

    if (sr_for_us(sr, packet + sizeof(struct sr_ethernet_hdr))) { /* for us */
        ...
        if (sr_is_icmp_echo(packet + sizeof(struct sr_ethernet_hdr),
            ...
            sr_set_ip_hdr(buf + sizeof(struct sr_ethernet_hdr), 5, 4, 0, ntohs(ip_hdr_recv->ip_len),
                ip_id_num /* ?? */, 0, 64, ip_protocol_icmp, ntohl(ip_hdr_recv->ip_dst), ntohl(ip_hdr_recv->ip_src));
            ip_id_num++;
            ...
            sr_set_ip_hdr(buf + sizeof(struct sr_ethernet_hdr), 5, 4, 0, buf_len - sizeof(struct sr_ethernet_hdr),
                ip_id_num, 0 /* offset */, 64, ip_protocol_icmp  ,ntohl(src_iface->ip)/* src */, ntohl(ip_hdr_recv->ip_src)/* dst */);
            ip_id_num++;
        ...
        if (next_hop == NULL) {
            ...
            /* set icmp */
            sr_set_icmp3_hdr(buf + sizeof(struct sr_ethernet_hdr) + sizeof(struct sr_ip_hdr),
                3, 0, 1234/* mtu */, packet + sizeof(struct sr_ethernet_hdr));
        ...
        ip_hdr_send->ip_sum = cksum(buf + sizeof(struct sr_ethernet_hdr), sizeof(struct sr_ip_hdr));
...
int sr_is_icmp_echo(uint8_t* ip_packet, unsigned int len) 
{
    ...
    if (len < sizeof(struct sr_ip_hdr) + sizeof(struct sr_icmp_hdr)) {
        return 0;
    }
...
void sr_set_icmp3_hdr(uint8_t* buf, uint8_t type, uint8_t code, uint16_t next_mtu, uint8_t* data)
{
    struct sr_icmp_t3_hdr* icmp_hdr = (struct sr_icmp_t3_hdr*)buf;
    icmp_hdr->icmp_type = type;
    icmp_hdr->icmp_code = code;
    icmp_hdr->icmp_sum = 0;
    icmp_hdr->next_mtu = htons(next_mtu);
    memcpy(icmp_hdr->data, data, ICMP_DATA_SIZE);

    icmp_hdr->icmp_sum = cksum(buf, sizeof(struct sr_icmp_t3_hdr));
}
    ...
    uint16_t ip_id, uint16_t ip_off, uint8_t ip_ttl, uint8_t ip_p, uint32_t ip_src, uint32_t ip_dst)
{
    ...
    ip_hdr->ip_sum = cksum(buf, sizeof(struct sr_ip_hdr));
}
